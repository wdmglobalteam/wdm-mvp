// --- filename: components/dashboard/EnrollmentCard.tsx ---
'use client';
 ...
 */
export default function EnrollmentCard({ hierarchy }: EnrollmentCardProps) {
	...
	const handleEnroll = async () => {
		...
		try {
			const response = await fetch('/api/enroll', {
				...
				credentials: 'same-origin', // <- IMPORTANT: include cookies so server reads the session
				body: JSON.stringify({
					pathId: hierarchy.id,
				}),
			});

			// If unauthorized, surface a friendly message
			if (response.status === 401) {
				setError('You must be signed in to enroll. Please sign in and try again.');
			...
			if (!response.ok) {
				...
				const msg = payload?.error ?? payload?.message ?? 'Enrollment failed';
				setError(String(msg));
				setIsEnrolling(false);
				return;
			}

			// Success - refresh server components to pick up new enrollment
			router.refresh();
		} catch (err) {
				...
				<div className="mb-8">
					...
					<div className="border-2 border-green-500 rounded-lg p-4 bg-green-950/20 mb-3">
						<div className="flex items-center justify-between">
							...
							<div className="bg-green-500 text-white text-xs px-3 py-1 rounded-full">Available</div>
						</div>
					...
					<div className="border border-gray-700 rounded-lg p-4 bg-gray-800/50 opacity-50">
						<div className="flex items-center justify-between">
							...
							<div className="bg-gray-600 text-gray-300 text-xs px-3 py-1 rounded-full">Coming Soon</div>
						</div>
					</div>
				</div>
